...
/**
 * IFlipAcceptance class
 * Coded by Clara Lasaosa, David Iglesias and Lucia Abascal.
 */
public class WFlipAcceptance {
	...
	// flipProbability Array for each neighbor and position level
	// Neighborhoods X flipProbability
	private double flipProbability[][] = null;
	...
	// neighbors position to obtain the flipProbability
	...
	public WFlipAcceptance (int[][] lattice, double []J, double H, int m) {
		this.J = J;
		...
		this.flipProbability = this.computeflipProbabilityArray(this.J);
	}
	...
	private ArrayList<Integer> radiusOfNeighbors(int NUMBER_OF_NEIGHBORS) {
		...
		//System.out.println("Getting for  " + NUMBER_OF_NEIGHBORS + " neighbors");
	...
	// Generate array of flipProbability for each neighbor:
	/**
	 * computeflipProbabilityArray
	 * Computes the probability due to flipProbability for each neighbor level and possible value of spin configuration.
	 * Possibles positions are (for 8 neighbors):
	 * Spin flipProbability probability
	 * -8	1
	 * -6	1
	 * -4	1
	 * -2	1
	 ...
	 *  6	exp(-2J*6)
	 *  8	exp(-2J*8)
	 * @param{double} 		J Array of J value for each neighbor level 
	 * @return{double[][]} 	A two dimension double array of neighbor level X flipProbability
	 */

	private double[][] computeflipProbabilityArray (double []J) {
		...
		// Compute flipProbability for each neighbors
		int len = numOfNeighbors.length;
		double[][] flipProbabilityArray = new double[len][];
		...
		for (int i = 0; i < len; i++) {
			...
			flipProbabilityArray[i] = new double[2 * this.MIDDLE_VALUE + 1];

			for (int j = -nOfNeighbors; j <= nOfNeighbors; j += 2) {
				
				if (j > 0) {
				
					flipProbabilityArray[i][this.MIDDLE_VALUE + j] = (double)Math.exp(-(double)2.0 * (double)J[i] * (double)j);
				} else {
				
					flipProbabilityArray[i][this.MIDDLE_VALUE + j] = 1;

				}
				
				if(debug) {
					//System.out.format("i = %d, J[i] = %f, nOfNeighbors = %d\n",i,  J[i], nOfNeighbors);
					//System.out.format("flipProbability j = %d , E = %f\n", j, flipProbabilityArray[i][this.MIDDLE_VALUE + j]);
				}
			}
		}

		return flipProbabilityArray;
	...
	public void checkFlip (int xCoor, int yCoor) {
		...
		int ici = this.lattice[xCoor][yCoor];
		
		// Compute probability for each neighborhood
		for (int i = 0; i<this.NUM_OF_NEIGHBORS; i++) {
			
			// For each neighborhood, compute the flipProbability (get the value of each neighbor)
			int NUM_OF_NEIGH = this.numOfNeighborsArray[i];
			
			
			int ien = 0;
			for (int j = 0; j<NUM_OF_NEIGH; j++ ) {
				int xPos = xCoor + this.neighborsPosition.get(neightborPositionPointer++);
				int yPos = yCoor + this.neighborsPosition.get(neightborPositionPointer++);
				
				//System.out.println(dx + "," + dy);
				
				// Ciclic boundary conditions
				// TODO: in order to increase the performance, precompute this boundary conditions...
				if (xPos >= this.L) {
					xPos -= this.L;
				}
			...
			totalProbability *= this.flipProbability[i][this.MIDDLE_VALUE + ien * ici];
		...
		// If we accept
		if (this.RandomGenerator.nextDouble() < totalProbability) {
			
						
			// Flip Spin
			lattice[xCoor][yCoor] = -ici;
